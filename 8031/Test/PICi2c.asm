;======================================================================
; I2C test program
; Drives I2C pins for testing I2C software on PIC16F818 board.
; Runs under Elector 8032 board.
;======================================================================
...
;======================================================================
            ...
            .equ user,rambase+256   ; User code entry point at 0x4100
...
loop:		lcall i2cstr	; start
		...
		mov a,#Slave
		lcall i2ctx	; Send i2C address as WRITE
		
		mov a,#0	; write smb register address
		lcall i2ctx

		mov a,counter
		inc a
		mov counter,a
		lcall i2ctx	; send data byte
...
;		mov a,#slave+1	; read from slave
;		lcall i2ctx
;		setb c		; want to send NACK as only read 1 byte
;		lcall i2crx	; read single byte
;		lcall scomtx8	; transmit
;            	mov dptr,#crlf
;	        lcall scomtxc
            	sjmp loop        

startmsg:	.db "PIC16F818 I2C test\r\n\0" 
crlf:       .db "\r\n\0"
;======================================================================
; scominit
