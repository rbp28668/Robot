;======================================================================
; I2C test program
; Tests SRF08 ultrasonic ranger and sends results back.
;======================================================================
...
loop:		lcall i2cstr	; start
			mov a,#ranger
			lcall i2ctx
			mov a,#0		; command register
			lcall i2ctx
			mov a,#rng_cm	; initiate ranging in cm
			lcall i2ctx
			lcall i2cstp

wtrng:		lcall i2cstr	; start
			mov a,#ranger	; write address
			lcall i2ctx
			mov a,#0		; software revision register
			lcall i2ctx

			lcall i2cstr	; restart
			
			mov a,#ranger+1	; read address
			lcall i2ctx			
			setb c			; NACK as one byte read
			lcall i2crx

			cjne a,#0ffh,gotrng ; exit loop if responding (not FF)
			sjmp wtrng

gotrng:		lcall i2cstr	; start
			mov a,#ranger	; write address
			lcall i2ctx
			mov a,#1		; light sensor
			lcall i2ctx

			lcall i2cstr	; restart

			mov a,#ranger+1	; read from ranger
			lcall i2ctx
			clr c
			lcall i2crx		; read light sensor
			lcall scomtx8	; transmit
			mov a,#'-'
			lcall scomtxa
			clr c
			lcall i2crx		; read 1st echo high
			lcall scomtx8
			setb c			; want to send NACK as last byte read
			lcall i2crx		; read 1st echo low
			lcall scomtx8	; transmit
            mov dptr,#crlf
            lcall scomtxc
            sjmp loop        

startmsg:	.db "CSFR08 ranging module test\r\n\0" 
crlf:       .db "\r\n\0"


;======================================================================
; rdcomp
; Reads the CMPS03 and returns bearing 0 to FF in A
;======================================================================

rdcomp:		lcall i2cstr	; start
			mov a,#compas
			lcall i2ctx
