...
;-- System variables at start of general purpose memory area
            ...
            .equ    servopos,   varbase + 2   ; byte containing servo position
            .equ    countdn,    varbase + 3   ; countdown byte for timing
            .equ    collatch,   varbase + 4   ; byte for latching collision sensors
            .equ    tickvect,   varbase + 5   ; 2 bytes for tick chaining vector (low,high)
            .equ    vpscratc,   varbase + 7   ; 8 bytes for virtual machine workspace					 
            .equ    dramaloc,   varbase + 15  ; 2 bytes for top of allocated data mem ptr
            .equ    state,      varbase + 17  ; 2 bytes for current state (pt to event records)
            .equ    stcnt,      varbase + 19  ; 1 byte for count of event records for this state
            .equ    vmfltc,     varbase + 21  ; 1 byte Virtual machine fault code.
            .equ    vmrstk,     varbase + 22  ; 2 bytes virtual machine return stack base.
            .equ    events,     varbase + 24  ; 2 byte pointer to event table
            .equ    stable,     varbase + 26  ; 2 byte pointer to state table
			.equ    stemap,		varbase + 28  ; 2 byte pointer to the event map for the current state.
            .equ    image,      varbase + 30  ; 2 byte pointer to image being executed.
